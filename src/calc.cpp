...
char * bracketsConvert(const char *expression, char *tmpPtr, char *newExp, long long *right, long long *newExpEnd, int *ERROR_CODE) {
            ...
            if (!--bracketsCount) {
                ...
                *right = i - 1; //Присваиваем правой границе индекс последней закрывающей скобки
                if (bracketsMax == 1) { // Если существует 1 открывающая и 1 закрывающая, то необходимо посчитать выражение
                    ...
                    tmpPtr = calc(tmpExpression, *right, ERROR_CODE, 1);
                    return tmpPtr;
                } else {
                    tmpPtr += flag;
